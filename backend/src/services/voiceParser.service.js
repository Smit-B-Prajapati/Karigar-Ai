...
export function detectItem(text = '') {
  ...
  if (/સાડી|સાડીઓ|સાડીની|સાડીનું|પટોળું|પટોળાં|कांजीवरम|साड़ी|सड़ी|saree|sari|patola/i.test(lower)) {
  ...
  if (/સૂટ|સલવાર|સલવાર સૂટ|सूट|सलवार|salwar|suit/i.test(lower)) {
  ...
  if (/રાખડી|રાખડીઓ|રાખી|રાખીઓ|राखी|राखड़ी|rakhi|rakhri|rakshabandhan|रक्षाबंधन/i.test(lower)) {
...
export function detectMaterial(text = '', detectedItem = null) {
  ...
  if (/ચામડું|ચામડા|ચર્મ|चमड़ा|leather/i.test(lower)) {
    return {
      noun: 'Leather',
...
export function detectSpecialCraft(text = '') {
  ...
  if (/નક્શીકામ|નકશી|नक्काशी|carving|hand carved/i.test(lower)) {
...
/**
 * Call Gemini AI to parse natural language transcript into structured craft fields
 */
async function callGeminiVoiceParsing(transcript, language) {
  const model = config.aiModel || 'gemini-1.5-flash';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${config.geminiApiKey}`;

  const prompt = `
You are an expert Indian artisan product cataloguer for KarigarAI.
An artisan spoke the following product description (Spoken Language: ${language || 'Hindi/Gujarati/English'}):

"${transcript}"

Extract structured product fields from this spoken text.
CRITICAL RULES FOR EXTRACTION:
1. "name": MUST BE A SHORT, CATCHY 2 TO 4 WORD PRODUCT TITLE IN ENGLISH ONLY (e.g., "Handmade White Cotton Dress", "Pure Silk Bandhani Dupatta", "Hand-Painted Terracotta Vase", "Handmade Artisan Tote Bag"). Even if the artisan spoke in Hindi or Gujarati, the title MUST BE TRANSLATED TO CLEAN ENGLISH ONLY. NEVER output Devanagari, Gujarati, or any non-English script for "name". Max 4 words.
2. "category": Must match ONE of these exact categories: ${KNOWN_CATEGORIES.join(', ')}.
3. "material": Extract exact material mentioned or implied in English (e.g., "Pure Cotton Fabric", "Pure Mulberry Silk", "Solid Brass Metal", "Carved Teak Wood", "Natural Terracotta Clay").
4. "craftType": Extract craft technique in English (e.g., "Handcrafted Tailoring", "Hand-Tied Threadwork", "Wheel Pottery", "Traditional Block Printing", "Hand Embroidery", "Bandhani Tie & Dye").
5. "price": Extract selling price number (e.g., 750).
6. "materialCost": Extract material cost number (e.g., 450).
7. "description": Clean, polished 2-3 sentence craft story based on spoken text.

Return ONLY a valid raw JSON object with keys: name, category, material, craftType, price, materialCost, description.
`;
  ...
  if (!response.ok) {
    throw new Error(`Gemini Voice Parsing API Error: ${response.status}`);
  }

  const data = await response.json();
  const rawText = data?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!rawText) throw new Error('Empty response from AI voice parser');

  const jsonString = rawText.replace(/```json/g, '').replace(/```/g, '').trim();
  const parsed = JSON.parse(jsonString);

  // Strictly sanitize title to short English only
  parsed.name = sanitizeShortEnglishTitle(parsed.name, parsed.category, parsed.material);
  return parsed;
}

/**
 * Smart heuristic/regex parser fallback for offline or un-keyed environments
 */
function heuristicVoiceParsing(transcript) {
  ...
  const parsedPrice = priceMatches ? priceMatches[1] : '750';
  ...
  const parsedCost = costMatches ? costMatches[1] : '450';
  ...
  if (detectedItem && detectedItem.category) {
    ...
    parsedCategory = 'Paintings & Folk Art';
  } else if (/jewelry|jewel|metal|brass|silver|gold|દાગીના|गहने|પીતળ/i.test(lower)) {
    parsedCategory = 'Jewelry & Metalware';
  }

  // Explicit category voice prompt detection (e.g. "category is X")
  ...
  if (explicitCategoryMatch) {
    ...
    if (matchedCategoryStr.includes('festiv')) parsedCategory = 'Festive Craft';
    else if (matchedCategoryStr.includes('textile') || matchedCategoryStr.includes('apparel')) parsedCategory = 'Textiles & Apparel';
    else if (matchedCategoryStr.includes('wood')) parsedCategory = 'Wood Crafts';
    else if (matchedCategoryStr.includes('bag')) parsedCategory = 'Bags & Accessories';
    else if (matchedCategoryStr.includes('paint') || matchedCategoryStr.includes('art')) parsedCategory = 'Paintings & Folk Art';
    else if (matchedCategoryStr.includes('jewel') || matchedCategoryStr.includes('metal')) parsedCategory = 'Jewelry & Metalware';
    else if (matchedCategoryStr.includes('pottery') || matchedCategoryStr.includes('ceramic')) parsedCategory = 'Pottery & Ceramics';
  }

  // 4. Material Resolution
  let parsedMaterial = 'Artisan Craft Material';
  if (detectedMat && detectedMat.en) {
    parsedMaterial = detectedMat.en;
  } else if (/cotton|kapas|કોટન|સુતરાઉ|कपास|कॉटन|सूती/i.test(lower)) {
    parsedMaterial = parsedCategory === 'Textiles & Apparel' ? 'Pure Cotton Fabric' : 'Pure Cotton';
  } else if (/silk|resham|સિલ્ક|રેશમ|सिल्ક|रेशम/i.test(lower)) {
    parsedMaterial = 'Pure Mulberry Silk';
  } else if (/clay|terracotta|mitti|માટી|मिट्टी/i.test(lower)) {
    parsedMaterial = 'Natural Terracotta Clay';
  } else if (/brass|pittal|પીતળ|पीतल/i.test(lower)) {
    parsedMaterial = 'Solid Brass Metal';
  } else if (/wood|teak|lakdi|લાકડું|लकड़ी/i.test(lower)) {
    parsedMaterial = 'Carved Teak Wood';
  } else if (/jute|જૂટ|जूट/i.test(lower)) {
    parsedMaterial = 'Eco Jute Fibre';
  }

  // 5. Craft Technique Resolution
  let parsedCraftType = 'Handcrafted Technique';
  if (detectedSpecial && detectedSpecial.technique) {
    parsedCraftType = detectedSpecial.technique;
  } else if (detectedItem && detectedItem.technique) {
    parsedCraftType = detectedItem.technique;
  } else if (detectedMat && detectedMat.technique) {
    parsedCraftType = detectedMat.technique;
  }

  // 6. Short & English-Only Product Title Generation
  const parsedName = generateEnglishCraftTitle(text, parsedCategory, parsedMaterial);

  return {
    name: parsedName,
    category: parsedCategory,
    material: parsedMaterial,
    craftType: parsedCraftType,
    price: parsedPrice,
 ...
 */
export async function parseVoiceTranscriptToFields(transcript, language = 'en-IN') {
  if (!transcript || typeof transcript !== 'string') {
    throw new Error('Transcript text is required for voice field parsing.');
  }

  // 1. Try Gemini AI parsing if API key present
  if (config.geminiApiKey && config.geminiApiKey.trim() !== '') {
    try {
      const aiResult = await callGeminiVoiceParsing(transcript, language);
      return {
        success: true,
        extracted: aiResult,
        engine: 'gemini-ai-entity-extractor'
      };
    } catch (err) {
      console.warn('Gemini Voice Parsing failed, fallback to heuristic parser:', err.message);
    }
  }

  // 2. Heuristic fallback
  const fallbackResult = heuristicVoiceParsing(transcript);
  return {
    success: true,
    extracted: fallbackResult,
    engine: 'heuristic-multilingual-parser'
  };
}

export default {
  parseVoiceTranscriptToFields,
